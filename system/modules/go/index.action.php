...
class index extends base {
	...
	public function init(){		
		...
		//即将揭晓
		$shoplist=$this->db->GetList("select * from `@#_shoplist` where `q_uid` is null ORDER BY `shenyurenshu` ASC LIMIT 5");
		//人气商品
		$shoplistrenqi=$this->db->GetList("select * from `@#_shoplist` where `renqi`='1' and `q_uid` is null ORDER BY id DESC LIMIT 5");
	...
	*/
	public function glist(){
		...
		if(!$cid){
			...
			$brand=$this->db->GetList("select id,cateid,name from `@#_brand` where `cateid`='$cid' order by `order` DESC");   
