...
class getshop extends SystemAction {
	...
	public function lottery_shop_json(){
		...
		if(!$gid){
			$info = $db->GetOne("select qishu,xsjx_time,id,thumb,title,q_uid,q_user,q_end_time from `@#_shoplist` where `q_showtime` = 'Y' order by `q_end_time` ASC");
		}else{		
			$infos = $db->GetList("select qishu,xsjx_time,id,thumb,title,q_uid,q_user,q_end_time from `@#_shoplist` where `q_showtime` = 'Y' order by `q_end_time` ASC limit 0,4");			
			$gid = @explode('_',$gid);		
			$info = false;		
